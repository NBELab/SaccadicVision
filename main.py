...
log_dir = save_models_path+"logs/" + '_' + \
...
mc = tf.keras.callbacks.ModelCheckpoint(
    "training_checkpoints/cp.ckpt", save_best_only=True, save_weights_only=False, monitor='ssim', mode='max')
early_stopping = EarlyStoppingWMinEpoch(
    monitor='ssim', min_delta=0.005, mode='max', patience=20,  earliest_epoch=150)
activation_type = 4
activation_name = ['lerelu', 'relu', 'tanh', 'swish', 'mish'][activation_type]
activation = [tf.nn.leaky_relu, tf.nn.relu,
              tf.nn.tanh, swish, mish][activation_type]
...
train = DataGenerator(h5_path, inds1[:int(
    num_train1)], h=scene_resolution[0], w=scene_resolution[1], batch_size=batch_size)
val = DataGenerator(h5_path, inds1[int(num_train1):int(
    num_train1)+int(num_val1)], h=scene_resolution[0], w=scene_resolution[1], batch_size=batch_size)
test = DataGenerator(h5_path, inds1[-int(num_test1):],
                     h=scene_resolution[0], w=scene_resolution[1], batch_size=batch_size)


generator = make_generator_model()
discriminator = make_discriminator_model(
    (saccade_resolution[0], saccade_resolution[1]), 3)

model_path = save_models_path+'/'+model_name
os.makedirs(model_path, exist_ok=True)
keras.utils.plot_model(generator, to_file=model_path +
                       "/generator.png", show_shapes=True)
keras.utils.plot_model(discriminator, to_file=model_path +
                       "/discriminator.png", show_shapes=True)
...
cond_gan.build(
    (None, None, None, saccade_resolution[0], saccade_resolution[1]))
...
for i in range(n):
    ...
    lap_events_lst, laplacian_rec_lst, int_rec_lst, color_rec_lst, = cond_gan.predict(
        batch[0])
    ...
    for j in range(batch_size):
        ...
        rgb_real = opp2rgb_tf(opp_real_lst[j])
        ssim_intensity.append(tf.reduce_mean(tf.image.ssim(tf.expand_dims(
            opp_rec_lst[j, :, :, 2], -1), tf.expand_dims(opp_real_lst[j, :, :, 2], -1), 1, filter_size=5)).numpy())
    ...
    '''
        ssim_pred.append(tf.reduce_mean(tf.image.ssim(
            rgb_pred, rgb_real, 1, filter_size=5)).numpy())
    ...
    '''
        ...
        lap_events = norm_image(
            lap_events_lst[j, :, :, 0]).numpy().astype(np.float32)
