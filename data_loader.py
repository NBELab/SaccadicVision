...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, filename, all_inds, h=scene_resolution[0], w=scene_resolution[1], batch_size=8, shuffle=True, preload_data=False):
        ...
        self.h = h
        self.w = w
        self.mask = create_circular_mask_opp(saccade_resolution[0], saccade_resolution[1], radius=mask_rad)
        self.indexes = all_inds[:num_samples].astype(np.int32) if num_samples > 0 else all_inds.copy().astype(np.int32)
        self.shuffle = shuffle
        self.on_epoch_end()
        self.h5_file = tables.open_file(filename, 'r')
        self.data_set = self.h5_file.root.my_group
    ...
    def __getitem__(self, index):
        ...
        events, saccades = self.data_set.event_frames[indexes, ...], norm_image_np(self.data_set.saccades[indexes, ...])
        num_of_events = self.data_set.num_of_event_frames[indexes, ...]
        center_points = self.data_set.center_points[indexes, ...].astype(np.int32)
        gaze_points = self.data_set.gaze_points[indexes, ...].astype(np.int32)
        scenes = norm_image_np(self.data_set.scenes[indexes, ...].astype(np.int32))
        new_events = []
        ...
        for event in events:
            ...
            for m in range(num_of_saccades):
                ...
                for j in range(num_of_events[i][m]):
                    ...
                    if (j % 10 == 0 and j != 0) or j == num_of_events[i][m] - 1:
                        event_lst.append(real_world[self.crop_row_start:self.crop_row_end, self.crop_col_start:self.crop_col_end])
                        real_world = np.zeros((scene_resolution))
                num_of_events[i][m] = math.ceil(num_of_events[i][m] / 5)
                while len(event_lst) < 5:
        ...
        for i in range(num_of_saccades + 1):
            if i == 0:
                concatenated = np.concatenate((fov_arr_list[i], np.zeros_like(new_events[:, 0, :, :, :])), axis=1)
            else:
                concatenated = np.concatenate((fov_arr_list[i], new_events[:, i - 1, :, :, :]), axis=1)
        ...
        y_opp = tf.transpose(y_opp, perm=[0, 3, 1, 2])  # Convert to channels-first
        y_opp = y_opp[:, :, self.crop_row_start:self.crop_row_end, self.crop_col_start:self.crop_col_end]  # Crop to center
    ...
    def process_saccade_images(self, saccade_images, gaze_points, xc, yc, p, mask, channel_index):
        ...
        for at, img in enumerate(saccade_images):
            ...
            start_row = gaze_points[at, channel_index, 0] - foveat.shape[0] // 2
            start_col = gaze_points[at, channel_index, 1] - foveat.shape[1] // 2
            new_arr[start_row:start_row + foveat.shape[0], start_col:start_col + foveat.shape[1], :] = foveat
            foveat = new_arr[self.crop_row_start:self.crop_row_end, self.crop_col_start:self.crop_col_end]
