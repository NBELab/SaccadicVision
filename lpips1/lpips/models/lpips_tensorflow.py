...
def learned_perceptual_metric_model(image_size, vgg_model_ckpt_fn, lin_model_ckpt_fn):
    ...
    # merge two model
    input1 = Input(shape=(image_size[0], image_size[1], 3), dtype='float32', name='input1')
    input2 = Input(shape=(image_size[0], image_size[1], 3), dtype='float32', name='input2')
    ...
    # normalize
    net_out1 = [Lambda(lambda x: x /( 1e-10+ tf.math.sqrt(tf.reduce_sum(tf.square(x), axis=1, keepdims=True))))(t)
                for t in net_out1]
    net_out2 = [Lambda(lambda x: x /( 1e-10+tf.math.sqrt(tf.reduce_sum(tf.square(x), axis=1, keepdims=True))))(t)
                for t in net_out2]
...
def linear_model(input_image_size):
    ...
    for ii, channel in enumerate(vgg_channels):
        ...
        image_size_0 = input_image_size[0] // (2 ** ii)
        image_size_1 = input_image_size[1] // (2 ** ii)
        model_input = Input(shape=(channel, image_size_0, image_size_1), dtype='float32')
